...
export const onGet: RequestHandler = async ({ query }) => {
  ...
  console.log("creating human text..");
  const humanText = await generateToHuman(text);
  console.log(humanText);
  console.log("parsing blog..");
  const parsedBlog = parseBlog(humanText as string);
...
const generateText = async (keywords: string) => {
  const prompt = `I want to create a blog all of with the following keywords included: ${keywords}. Write a 800-word to 1000-word 100% Unique, Human-Written article in English. Write The article In Your Own Words Rather Than Copying And Pasting From Other Sources`;
  const response = await openai.chat.completions.create({
    model: "gpt-4",
    messages: [
      {
...
const generateToHuman = async (text: string | null) => {
  const format =
    "{title:*;intro:*;text1:*;text2:*;text3:*;text4:*;text5:*;text6:*;text7:*;}";

  const prompt = `Make this text more human. Start Text: ${text}. End Text. IMPORTANT: I want you to return the blog in json format. This is the format: ${format}. Replace the * for the output.`;
